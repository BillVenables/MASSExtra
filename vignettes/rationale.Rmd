...
mpg_gic <- step_down(mpg0, k = "bic")      ## simple backward elimination, mainly used for GLMMs
...
ggplot(Cars93) + aes(x = Weight, y = MPG.city, colour = Cylinders) + geom_point() +
   ...
   ylab("Miles per gallon (city driving)") + scale_colour_viridis_d()
```
...
```{r}
## exported only as an S3 method
MASSExtra:::makepredictcall.normalise  
## an exported helper, but with a hidden name
.normalise                             
## the scale() equivalent
zs                                     
## the boxplot scaling, should anyone want it!
zq                                     
```
