...
with other packages, such as `dplyr` the `select` function can easly
be masked, causing confusion with users.  `MASS::select` is rarely
...
in how they are programmed.  The following chain from the package code
sets out the steps.
